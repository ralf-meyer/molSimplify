...
def branch(molcif, main_paths,atoms_in_sbu, new_atoms=[]):
    print('=== in branch',new_atoms)
    original_atoms = atoms_in_sbu.copy()
    for atom in new_atoms:
        bonded_list = molcif.getBondedAtoms(atom)
        print(bonded_list)
        print('sbu',len(set(bonded_list)-set(atoms_in_sbu)))
        print('path',len(set(bonded_list)-set(main_paths)))
...
def make_MOF_SBU_RACs(SBUlist, SBU_subgraph, molcif, depth, name,cell,anchoring_atoms, sbupath=False, connections_list=False, connections_subgraphlist=False, linkerpath=False):
    ...
    Gdi = nx.from_numpy_matrix(molcif.graph,create_using=nx.DiGraph)
    ...
    flat_SBU_list = [item for sublist in subcycle_list for item in sublist]
    ...
    for i, SBU in enumerate(SBUlist):
        ...
        main_paths = list(set(main_paths))+list(molcif.findMetal())
        SBU_mol = mol3D()
        tuple_list = []
        for val in SBU:
            ...
            if not in_cycles:
                bonded_atoms_to_val = molcif.getBondedAtoms(val)
                bonded_atoms_to_val_symbols = [molcif.getAtom(subval).symbol() for subval in bonded_atoms_to_val]
                for enum_i, bonded_atoms_non_ring in enumerate(bonded_atoms_to_val):
                    if (molcif.getAtom(int(bonded_atoms_non_ring)).symbol() == 'H'):
                        atoms_in_sbu.append(bonded_atoms_non_ring)
                        SBU_mol.addAtom(molcif.getAtom(bonded_atoms_non_ring))
                    if (molcif.getAtom(int(bonded_atoms_non_ring)).symbol() == 'O'):
                        atoms_in_sbu.append(bonded_atoms_non_ring)
                        SBU_mol.addAtom(molcif.getAtom(bonded_atoms_non_ring))
                    if (bonded_atoms_non_ring in main_paths) and (not (bonded_atoms_non_ring in SBU)):
                        print(bonded_atoms_non_ring,'===in nonpath part')
                        atoms_in_sbu.append(bonded_atoms_non_ring)
                        ...
                        new_atom = atom3D(Sym='X',xyz=temp_atom_coords)
                        SBU_mol.addAtom(new_atom)
                        tuple_list.append((SBU_mol.natoms-1,non_ring_added_atom_index))
                ...
                for atom in cycle:
                    ...
                    for ringatom_connected_atoms in molcif.getBondedAtoms(atom):
                        ...
                        if (ringatom_connected_atoms in main_paths) and (not (ringatom_connected_atoms in cycle)):
                            atoms_in_sbu.append(ringatom_connected_atoms)
                            atoms_that_are_X.append(added_atom_index)
                            temp_atom = molcif.getAtom(ringatom_connected_atoms)
                            temp_atom_coords = temp_atom.coords()
                            new_atom = atom3D(Sym='X',xyz=np.array(temp_atom_coords))
                            SBU_mol.addAtom(new_atom)
                            tuple_list.append((SBU_mol.natoms-1,added_atom_index))
                    print('b4',SBU_mol.natoms,len(atoms_in_sbu))
                    additional_branched_atoms,_ = branch(molcif,  main_paths, atoms_in_sbu.copy(),[atom])
                    for branched_atom in additional_branched_atoms:
                        # This gets anything that is part of a ring, but on a branch
                        if branched_atom not in atoms_in_sbu:
                            atoms_in_sbu.append(branched_atom)
        ...
        G_sbu = nx.from_numpy_matrix(SBU_mol.graph,create_using=nx.DiGraph)
        aperiodic = nx.is_aperiodic(G_sbu)
        print('====',aperiodic)
        is_periodic = not aperiodic
        ...
        # [SBU_mol.BCM(val[0],val[1],0.75) for val in tuple_list]
    ...
    for i, linker in enumerate(connections_list):
        ...
        print(linker_mol_adj_mat.shape,linker_mol.natoms)
