...
def simple_slope_ann(slope_excitation):
    globs=globalvars()
    path_to_file = resource_filename(Requirement.parse("molSimplify"),"molSimplify/python_nn/" + "final_slope")
    #print('path to ANN data: ',path_to_file)
    n = simple_network_builder([24,50,50],"final_slope") ## no alpha value
    slope_excitation,sl_center,sl_shift = slope_excitation_standardizer(slope_excitation)
...
def excitation_standardizer(excitation):
    ...
    centers = csv_loader("split_centers.csv")
    shifts = csv_loader("split_scales.csv")
...
def slope_excitation_standardizer(slope_excitation):
    ## this function implements 
    ## a scale-and-center
    ## type of normalization
    ## that may help predictions 
    ## currently testing for
    ## splitting andd slope only 
    centers = csv_loader("slope_centers.csv")
    shifts = csv_loader("slope_scales.csv")
    descriptor_centers = np.array(centers[1:])
    descriptor_shifts = np.array(shifts[1:])
    sl_center =  centers[0]
    sl_shift =  shifts[0]   
    slope_excitation = np.array(slope_excitation) 
    #print(slope_excitation) 
    #print('********************')
    #print(descriptor_centers)
    #print(descriptor_centers)
    #print('********************')

    slope_excitation = (slope_excitation - descriptor_centers)
    slope_excitation = np.divide(slope_excitation,descriptor_shifts) 
    #print(slope_excitation)
    return(slope_excitation,sl_center,sl_shift )
