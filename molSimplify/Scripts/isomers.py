...
def generatestereo(collapsed_representation):
    ...
    for isomer in stereoisomers:
        stereo = copy(isomer)
        stereo[0],stereo[2] = stereo[2],stereo[0] #flip ligand 1 and ligand 3 to generate a stereoisomer
        if not stereo[4].endswith('_alphabond') and not stereo[4].endswith('_betabond'): #skip rotation if sites 4 and 5 are bonded
    ...
    for atom in connecting_atoms:
        ...
        while finding_atoms == True:
            ...
            for atoms in current_sphere:
                next_sphere = next_sphere | set(ligand.getBondedAtoms(atoms)) #get a set containing elements from both sets
